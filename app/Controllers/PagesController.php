...
class PagesController extends Controller
{
    ...
    public function home(RequestInterface $request, ResponseInterface $response)
    {
        ...
        $req = $this->container->pdo->prepare('select count(*)-1 from Annonce where statu=1');
        $req->execute();
        $count = $req->fetch();
        ...
        $this->render($response, 'layouts/home.twig', ["v" => $count[0] + 1, "req1" => $result, "cnx" => $test, "nbPage" => $pagination, "testA" => $testAd, "user" => $use]);
    }
    public function pages(RequestInterface $request, ResponseInterface $response, $args)
    {
        ...
        if (empty($NbPage)) {
            ...
            if ($NbPage < $count[0] / $limit) {
                $pagination = intval($count[0] / $limit);
                $req1 = $this->container->pdo->prepare(
                    'SELECT a.titre, a.image,v.ville,a.PrixA ,a.idAn,a.datePub , a.img ,a.visitor FROM Annonce a , User u ,`model` m ,`marque` ma ,`ville` v WHERE a.statu=1 and a.idVille=v.id  and a.idUser=u.id and a.idModel=m.idModel and m.idM=ma.idM order by a.datePub desc limit ' . $skip . ',' . $limit . ';'
                );
                $req1->execute();
                $result = $req1->fetchAll();
                $this->render($response, 'layouts/home.twig', ["v" => $count[0][0], "req1" => $result, "cnx" => $test, "nbPage" => $pagination, "page" => $NbPage, "testA" => $testAd, "user" => $use]);
            } else {
                return $response->withRedirect('/usedCars/public/index.php');
            }
        }
    ...
    public  function  getAfficherAnnonce(RequestInterface $request, ResponseInterface $response)
    {
        ...
        if ($req1->rowCount() > 0) {
            if (isset($_GET["idAnn"]) && $_GET["idAnn"] != "") {
                ...
                $this->render($response, 'layouts/AfficherAnnonce.twig', ["user" => $use, "t" => $result, "cnx" => $test, "testA" => $testAd, "img" => $images, "count" => $count]);
            }
        } else {
    ...
    public  function postModele(RequestInterface $request, ResponseInterface $response)
    {
        ...
        $req1->execute();
        $result = $req1->fetchAll();
        $this->render($response, 'layouts/AjouterModele.twig', ["user" => $use, 'v' => "<span class='$clasStat'>$stat</span>", "req" => $result, "cnx" => $test, "testA" => $testAd]);
    }
    public  function getModele(RequestInterface $request, ResponseInterface $response)
    {
        if (isset($_SESSION['user'])) {
            ...
            $req1->execute();
            $result = $req1->fetchAll();
            $this->render($response, 'layouts/AjouterModele.twig', ["user" => $use, "req" => $result, "cnx" => $test, "testA" => $testAd]);
        } else {
    ...
    public  function getMarque(RequestInterface $request, ResponseInterface $response)
    {
        if (isset($_SESSION['user'])) {
            ...
            $this->render($response, 'layouts/ajouterMarque.twig', ["user" => $use, "cnx" => $test, "testA" => $testAd]);
        } else {
            return $response->withRedirect('loginU');
        }
    }
    ...
    public function postMarque(RequestInterface $request, ResponseInterface $response)
    {
        if (isset($_POST["marque"])) {
            ...
            if ($row_cnt > 0) {
                echo "<script> alert('existe deja ');</script>";
                $this->render($response, 'layouts/ajouterMarque.twig', ["user" => $use, 'v' => "<span class='badge badge-danger'>Non Ajoute</span>", "cnx" => $test, "testA" => $testAd]);
            } else {
    ...
    public function getAjouteAn(RequestInterface $request, ResponseInterface $response)
    {
        if (isset($_SESSION['user'])) {
            ...
            $req11->execute();
            $result21 = $req11->fetchAll();
            $this->render($response, 'layouts/AjouterAnnonce.twig', ["user" => $use, "req" => $result11, "req1" => $result1, "testA" => $testAd, "cnx" => $testCnx, "ville" => $result21]);
        } else {
            return $response->withRedirect('loginU');
        }
    }
        ...
        if (isset($_SESSION['user'])) {
            ...
            if (isset($_POST["titre"])) {
                ...
                if ($row_cont > 0) {
                    ...
                    $this->render($response, 'layouts/AjouterAnnonce.twig', ["user" => $use, "existe" => $existe, "testA" => $testAd, "cnx" => $test, "req" => $result110, "req1" => $result10, "ville" => $result21]);
                } else {
                    $this->render($response, 'layouts/AjouterAnnonce.twig', ["user" => $use, "existe" => $existe, "testA" => $testAd, "cnx" => $test, "req" => $result110, "req1" => $result10, "ville" => $result21]);
                }
            }
    ...
    public function postRecherche(RequestInterface $request, ResponseInterface $response)
    {
        ...
        $req11->execute();
        $result21 = $req11->fetchAll();
        $req1 = $this->container->pdo->prepare('SELECT a.titre, a.image,v.ville,a.PrixA ,a.idAn,a.datePub,a.img,a.visitor FROM Annonce a , User u ,`model` m ,`marque` ma,`ville` v WHERE a.statu=1 and a.idUser=u.id and a.idModel=m.idModel and a.idVille=v.id  and m.idM=ma.idM ' . $select);
        $req1->execute();
        $result = $req1->fetchAll();
        $test = $this->TestConnection();
        $this->render($response, 'layouts/Recherche.twig', ["user" => $use, "req" => $result22, "req1" => $result, "cnx" => $test, "testA" => $testAd, "ville" => $result21]);
    }
    public function getRecherche(RequestInterface $request, ResponseInterface $response)
    {
        ...
        $this->render($response, 'layouts/Recherche.twig', ["user" => $use, "req" => $result22, "cnx" => $test, "testA" => $testAd, "ville" => $result21]);
    }
    ...
    public function getPageU(RequestInterface $request, ResponseInterface $response)
    {
        if (isset($_SESSION['user'])) {
            ...
            if ($do == "edit" && $Rowcount > 0) {
                ...
                $req11->execute();
                $result21 = $req11->fetchAll();
                return  $this->container->view->render($response, 'layouts/ModifierAnnonce.twig', ["user" => $use, "reqq" => $result21, "id" => $iditem, "r" => $result, "req" => $result11, "req1" => $result1, "cnx" => $test, "testA" => $testAd]);
            } else if (isset($_GET['do']) == "edit" &&  $Rowcount <= 0) {
                echo '<script> alert("erreur");</script>';
            }
            if ($do == "delete") {
            ...
            $result2 = $req1->fetchAll();
            // Requete Liste marque
            $this->container->view->render($response, 'layouts/PageUser.twig', ["user" => $use, "req" => $result2, "session" => $_SESSION['user'], "cnx" => $test, "testA" => $testAd]);
        } else
            $this->render($response, 'layouts/loginUser.twig');
    }
    ...
    public function getStatistique(RequestInterface $request, ResponseInterface $response)
    {
        if (isset($_SESSION['user'])) {
            if ($_SESSION['user'] == "admin") {
                $test = $this->TestConnection();
                ...
                $req3 = $this->container->pdo->prepare("SELECT count(*) FROM `Annonce` where statu=1");
                $req3->execute();
                $result3 = $req3->fetch();
                $req6 = $this->container->pdo->prepare("SELECT count(*) FROM `User`");
                $req6->execute();
                $result6 = $req6->fetch();
                $req4 = $this->container->pdo->prepare("SELECT count(*) FROM `Annonce` where statu=0");
                $req4->execute();
                $result4 = $req4->fetch();
                $req5 = $this->container->pdo->prepare("       SELECT sum(`visitor`) FROM `Annonce`           ");
                $req5->execute();
                $result5 = $req5->fetch();
                $req1 = $this->container->pdo->prepare("SELECT `titre`,`visitor` FROM `Annonce` where statu=1 order by visitor DESC limit 5");
                $req1->execute();
                $result1 = $req1->fetchAll();
                $req = $this->container->pdo->prepare("SELECT  `username`,count(*) as 'vues' FROM `Annonce` a,`User` u WHERE a.`idUser`=u.`id`  group by `idUser` order by vues DESC LIMIT 5");
                $req->execute();
                $result2 = $req->fetchAll();


                return $this->container->view->render($response, 'layouts/Statistique.twig', ["cnx" => $test, "testA" => $testAd, "topuser" => $result2, "topAn" => $result1, "nbAnnonce" => $result3, "nbAttend" => $result4, "visitors" => $result5, "users" => $result6]);
            }
        }
    }
    //Post inscri :
